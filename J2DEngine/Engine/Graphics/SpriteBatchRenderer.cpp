...
bool CSpriteBatchRenderer::Init(CDirectXFramework * aFramework)
{
	myFramework = aFramework;

	D3D11_BUFFER_DESC desc = { 0 };
	desc.ByteWidth = sizeof(SSprite) * MAX_AMOUNT_SPRITES_IN_SPRITEBATCH;
...
void CSpriteBatchRenderer::Render(std::vector<SSpriteBatchRenderCommand>& aSpritesToRender)
{
	...
	for (SSpriteBatchRenderCommand& batch : aSpritesToRender)
	{
		std::vector<SSprite> sprites;
		sprites.reserve(batch.amount);
		int actualAmountToRender = batch.amount;
		for (int i = 0; i < batch.amount; ++i)
		{
			if (batch.sprites[i].shouldRender == false)
			{
				actualAmountToRender -= 1;
				continue;
			}

			SSprite rc;
			rc.position = batch.sprites[i].position;
			rc.color = batch.sprites[i].color;
			rc.dimensions = { batch.sprites[i].dimensions.x, batch.sprites[i].dimensions.y };
			rc.rotation = batch.sprites[i].rotation;
			rc.textureRect = { batch.sprites[i].textureRect.topLeft , batch.sprites[i].textureRect.bottomRight };
			rc.scale = { batch.sprites[i].scale.x, batch.sprites[i].scale.y };
			sprites.push_back(rc);
		}

		if (actualAmountToRender > 0)
		{
			D3D11_MAPPED_SUBRESOURCE resource = { 0 };
			result = context->Map(myBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
			if (FAILED(result)) { continue; }
			memcpy(resource.pData, &sprites[0], sizeof(SSprite) * actualAmountToRender);
			context->Unmap(myBuffer, 0);

			context->PSSetShaderResources(0, 1, &batch.sprites[0].texture.myTexture);

			context->Draw(actualAmountToRender, 0);
		}
	}
}
